...
Sections claimed by a catalog grammar become typed records instead of rules. Paragraph- and bullet-grained entities (actions, crew claims, trauma conditions) are typed indexes over rule prose: the owning rule keeps the source-faithful text and the typed records carry sub-spans inside it. In catalog records, `>` blockquote paragraphs are split into `commentary` (designer guidance) with the quote markers removed; all other wording is preserved as printed.
...
- `fidelity`: locator anchoring to printed identity, physical table starts and raw-text equality, sub-span containment in owning rules, typed catalog counts (12/58/12/6/35/8), and exact rights metadata in the source record, manifest, and `llms-full.txt`.
...
- `test`: regression fixtures for the Fortitude heading override, commentary splitting, table spans, downtime summary exclusion, claim types, attribution survival, and search postings.
