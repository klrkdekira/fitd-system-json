.PHONY: help install extract manifest bundle llms-full search-index collection-index coverage vocab sitemap anomalies fidelity graph test validate schema determinism check
...
help:
	...
	@echo "  extract      Re-extract objects/ from Blades-in-the-Dark-SRD.md"
...
# silently rewritten.
check: determinism extract manifest bundle llms-full search-index collection-index coverage vocab sitemap anomalies test validate fidelity graph schema
