...
from fitdlib import BASE, COLLECTION_TYPES, MANIFEST_NAME, dump_json, iter_object_files, load_json
...
def main():
    ...
    for term, definition in context.items():
        if term.startswith("@") or term in ("fitd", "schema", "dcterms"):
            continue
        iri = resolve_term(term, context)
        if not iri.startswith(vocab_base):
            continue
...
<title>Blades in the Dark SRD System JSON Vocabulary</title>
<style>
...
<body>
<h1>Blades in the Dark SRD System JSON Vocabulary</h1>
