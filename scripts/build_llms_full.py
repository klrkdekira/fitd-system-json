...
def format_record(record) -> str:
    ...
    # avoids duplicate table cells while remaining a complete recursive
    # projection.
    selected = []
    seen = set()
...
def build(root: Path) -> None:
    ...
    out = [
        "# Blades in the Dark SRD System JSON — full corpus context",
        "",
        "Machine-readable JSON-LD 1.1 reference corpus for the Blades in the "
        "Dark SRD (Forged in the Dark rules, CC-BY-3.0).",
        f"Base IRI: {BASE}",
